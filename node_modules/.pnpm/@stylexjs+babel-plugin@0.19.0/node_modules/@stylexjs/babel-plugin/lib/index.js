  ...
  if (bailOut) {
    ...
    for (const argPath of argumentPaths) {
      ...
      function ObjectExpression(objPath) {
        const hasCssMarker = objPath.node.properties.some(prop => t__namespace.isObjectProperty(prop) && (t__namespace.isIdentifier(prop.key) && prop.key.name === '$$css' || t__namespace.isStringLiteral(prop.key) && prop.key.value === '$$css') && t__namespace.isBooleanLiteral(prop.value, {
          value: true
        }));
        if (hasCssMarker) {
...
function requireBabelTransform () {
	...
	function getPropKey(prop, computed) {
	  if (!computed && t$1.isIdentifier(prop)) {
	    return prop.name;
	  }
	  if (computed && t$1.isStringLiteral(prop)) {
	    return prop.value;
	  }
	  if (computed && t$1.isNumericLiteral(prop)) {
	...
	function compileDynamicStyle(path, styleInfo, state, compile) {
	  ...
	  const param = t$1.identifier('_v');
	  const nullCheck = t$1.binaryExpression('!=', param, t$1.nullLiteral());

	  const compiledObj = t$1.objectExpression([
	    t$1.objectProperty(
	      t$1.stringLiteral(propKey),
	      t$1.conditionalExpression(nullCheck, t$1.stringLiteral(className), param),
	    ),
	    t$1.objectProperty(t$1.stringLiteral('$$css'), t$1.booleanLiteral(true)),
	  ]);

	  const inlineVarsObj = t$1.objectExpression([
	    t$1.objectProperty(
	      t$1.stringLiteral(varName),
	      t$1.conditionalExpression(
	        t$1.binaryExpression('!=', param, t$1.nullLiteral()),
	        param,
	        t$1.identifier('undefined'),
	      ),
	    ),
	  ]);

	  const fnBody = t$1.arrayExpression([compiledObj, inlineVarsObj]);
	  const arrowFn = t$1.arrowFunctionExpression([param], fnBody);

	  const stylesObj = t$1.objectExpression([
	    t$1.objectProperty(t$1.identifier(property), arrowFn),
	  ]);

	  const hoistedId = compile.hoistExpression(path, stylesObj);

	  const callExpr = t$1.callExpression(
	    t$1.memberExpression(hoistedId, t$1.identifier(property)),
	    [value],
	  );
    ...
    visitor: {
      ...
      JSXOpeningElement(path) {
        ...
        for (const attr of path.get('attributes')) {
          ...
          attr.replaceWith(t__namespace.jsxSpreadAttribute(t__namespace.callExpression(t__namespace.memberExpression(t__namespace.identifier(stylexLocalName), t__namespace.identifier('props')), [value.node])));
          break;
        }
      },
      CallExpression(path) {
...
function getStylexRuntimeBinding(path, state) {
  ...
  const [importPath] = program.unshiftContainer('body', t__namespace.importDeclaration([t__namespace.importNamespaceSpecifier(t__namespace.identifier(stylexLocalName))], t__namespace.stringLiteral(importSource)));
