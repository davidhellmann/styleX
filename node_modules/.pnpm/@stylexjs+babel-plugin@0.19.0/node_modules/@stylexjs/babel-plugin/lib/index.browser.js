...
function hoistExpression(path, astExpression) {
  ...
  programStatementPath.insertBefore(variableDeclaration('const', [variableDeclarator(hoistedIdent, astExpression)]));
  return hoistedIdent;
}
function pathReplaceHoisted(path, astExpression) {
  ...
  programStatementPath.insertBefore(variableDeclaration('const', [variableDeclarator(nameIdent, astExpression)]));
...
function addNamedImport(statementPath, as, from, options) {
  ...
  lastImport.insertAfter(variableDeclaration('var', [variableDeclarator(importName, identifier)]));
  return importName;
}
...
class StateManager {
  ...
  stylexDefineVarsNestedImport = new Set();
  stylexDefineConstsNestedImport = new Set();
  stylexCreateThemeNestedImport = new Set();
  stylexConditionalImport = new Set();
  atomImports = new Map();
      ...
      case 'commonJS':
        {
          ...
          const resolvedFilePath = filePathResolver(importPath, sourceFilePath, aliases, this.options.unstable_moduleResolution?.rootDir);
  ...
  registerStyles(styles, path) {
    ...
    for (const [_key, styleObj, priority] of styles) {
      ...
      const properties = [objectProperty(identifier('ltr'), stringLiteral(ltr)), ...(rtl != null ? [objectProperty(identifier('rtl'), stringLiteral(rtl))] : []), objectProperty(identifier('priority'), numericLiteral(priority)), ...(constKey != null ? [objectProperty(identifier('constKey'), stringLiteral(constKey)), objectProperty(identifier('constVal'), typeof constVal === 'number' ? numericLiteral(constVal) : stringLiteral(String(constVal)))] : [])];
      statementPath.insertBefore(expressionStatement(callExpression(injectName, [objectExpression(properties)])));
    }
  }
...
const filePathResolver = (relativeFilePath, sourceFilePath, aliases, rootDir) => {
  for (const importPathStr of getPossibleFilePaths(relativeFilePath)) {
    if (importPathStr.startsWith('.')) {
      try {
        return url$1.fileURLToPath(moduleResolve(importPathStr, url$1.pathToFileURL(sourceFilePath)));
      } catch {
        continue;
      }
    }
    ...
    for (const possiblePath of allAliases) {
      if (possiblePath.startsWith('/ROOT/') && rootDir != null) {
        const realPath = path.join(rootDir, possiblePath.slice('/ROOT/'.length));
      ...
      try {
        return url$1.fileURLToPath(moduleResolve(possiblePath, url$1.pathToFileURL(sourceFilePath)));
      } catch {
        continue;
      }
    }
  ...
  if (ATOMS_SOURCES.has(sourcePath)) {
    for (const specifier of node.specifiers) {
      if (specifier.type === 'ImportNamespaceSpecifier') {
        ...
        state.atomImports.set(specifier.local.name, '*');
      } else if (specifier.type === 'ImportSpecifier' && (specifier.imported.type === 'Identifier' || specifier.imported.type === 'StringLiteral')) {
...
function readRequires(path, state) {
  ...
  if (init != null && init.type === 'CallExpression' && init.callee?.type === 'Identifier' && init.callee?.name === 'require' && init.arguments?.length === 1 && init.arguments?.[0].type === 'StringLiteral' && ATOMS_SOURCES.has(init.arguments[0].value)) {
    if (node.id.type === 'Identifier') {
...
const invalidDefineVarsFunctionValue = () => 'Function values in defineVars() must be zero-argument and return a static value supported by defineVars().';
const cyclicDefineVarsReference = cycle => `Cyclic same-group references in defineVars() are not allowed: ${cycle}.`;
const unknownDefineVarsReference = (key, dependency) => `Unknown same-group reference "${dependency}" found while resolving "${key}" in defineVars().`;
...
const LINT_UNCLOSED_STRING = 'Rule contains an unclosed string';
...
var m = /*#__PURE__*/Object.freeze({
  ...
  LINT_UNCLOSED_STRING: LINT_UNCLOSED_STRING,
  ...
  cyclicDefineVarsReference: cyclicDefineVarsReference,
  illegalArgumentLength: illegalArgumentLength,
  invalidDefineVarsFunctionValue: invalidDefineVarsFunctionValue,
  ...
  unknownDefineVarsReference: unknownDefineVarsReference
});
...
function detectUnclosedStrings(ast, _) {
  ast.walk(node => {
    if (node.type === 'string' && node.unclosed) {
      throw new Error(LINT_UNCLOSED_STRING);
    }
  });
  return ast;
}
...
const normalizers = [detectUnclosedFns, detectUnclosedStrings, normalizeWhitespace, normalizeTimings, normalizeZeroDimensions, normalizeLeadingZero, normalizeQuotes, convertCamelCasedValues];
...
const SEPARATOR = '.';
function toVarsConfigValue(value) {
  if (typeof value === 'string') {
    return value;
  }
  if (typeof value === 'object' && value != null && !Array.isArray(value)) {
...
function flattenImpl(obj, prefix, result, isLeaf, transformLeaf) {
  for (const key of Object.keys(obj)) {
    if (key.includes(SEPARATOR)) {
      throw new Error(`Key "${key}" must not contain the "${SEPARATOR}" character. ` + 'Use nested objects instead of dots in key names. ' + 'See: https://www.designtokens.org/tr/drafts/format/#character-restrictions');
    }
    const value = obj[key];
    const fullKey = prefix ? `${prefix}${SEPARATOR}${key}` : key;
    if (isLeaf(value)) {
      result[fullKey] = transformLeaf ? transformLeaf(value) : value;
    } else if (typeof value === 'object' && value != null && !Array.isArray(value)) {
      flattenImpl(value, fullKey, result, isLeaf, transformLeaf);
    }
  }
}
function isVarsLeaf(value) {
...
function isStringLeaf(value) {
  return typeof value === 'string';
}
function isConstsLeaf(value) {
  return typeof value === 'string' || typeof value === 'number';
}
function transformVarsLeaf(value) {
  if (typeof value === 'string') return value;
  if (isCSSType(value)) return value;
  return toVarsConfigValue(value);
}
function transformOverridesLeaf(value) {
  if (typeof value === 'string') return value;
  if (isCSSType(value)) return toVarsConfigValue(value.value);
  return toVarsConfigValue(value);
}
function flattenNestedVarsConfig(obj, prefix = '') {
  const result = {};
  flattenImpl(obj, prefix, result, isVarsLeaf, transformVarsLeaf);
  return result;
}
function flattenNestedOverridesConfig(obj, prefix = '') {
  const result = {};
  flattenImpl(obj, prefix, result, isVarsLeaf, transformOverridesLeaf);
  return result;
}
function flattenNestedStringConfig(obj, prefix = '') {
  const result = {};
  flattenImpl(obj, prefix, result, isStringLeaf);
  return result;
}
function flattenNestedConstsConfig(obj, prefix = '') {
  const result = {};
...
function styleXDefineVarsNested(nestedVariables, options) {
...
function styleXDefineConstsNested(nestedConstants, options) {
...
function styleXCreateThemeNested(themeVars, nestedOverrides, options) {
  const {
    __varGroupHash__,
    ...nestedVarRefs
  } = themeVars;
...
const defineVarsNested = styleXDefineVarsNested;
const defineConstsNested = styleXDefineConstsNested;
const createThemeNested = styleXCreateThemeNested;
...
function addSourceMapData(obj, babelPath, state) {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    const styleNodePath = babelPath.get('arguments.0.properties').find(prop => {
      ...
      return isIdentifier(k) && k.name === key || isStringLiteral(k) && k.value === key || isNumericLiteral(k) && String(k.value) === key;
    });
    if (styleNodePath) {
  ...
  for (const path of binding.referencePaths) {
    ...
    if (parentPath.isMemberExpression() && parentPath.node.object === path.node) {
      ...
      if (parent.isCallExpression() && parent.node.callee === memberExpr.node) {
        ...
        if (isIdentifier(property) && MUTATING_ARRAY_METHODS.has(property.name)) {
          return true;
        }
      }
    }
...
function getVarGroupHash(fileName, exportName, traversalState) {
  return traversalState.options.classNamePrefix + utils.hash(utils.genFileBasedIdentifier({
    fileName,
    exportName
  }));
}
function resolveVarGroupKey(key, fileName, exportName, traversalState) {
  if (key.startsWith('--')) {
    return `var(${key})`;
  }
  ...
  const varSafeKey = (key[0] >= '0' && key[0] <= '9' ? `_${key}` : key).replace(/[^a-zA-Z0-9]/g, '_');
  const varName = debug && enableDebugClassNames ? `${varSafeKey}-${classNamePrefix}${utils.hash(strToHash)}` : classNamePrefix + utils.hash(strToHash);
  return `var(--${varName})`;
}
function createVarGroupProxy({
  fileName,
  exportName,
  traversalState,
  onAccess
...
function evaluateImportedFile(filePath, namedExport, state, bindingPath) {
  ...
  const ast = parseSync(fileContents, {
    babelrc: true
  });
  if (!ast || ast.errors || !isNode(ast)) {
...
function _evaluate(path, state) {
  ...
  if (path.isArrowFunctionExpression()) {
    ...
    if (body.isExpression() && identParams.length === params.length) {
      const evaluatedExpr = body;
      const evaluatedFn = (...args) => {
  ...
  function getFullMemberPath(memberPath) {
    const parts = [];
      ...
      if (importPath && importPath.isImportDeclaration() && !state.functions.disableImports) {
        ...
        if (state.confident) {
          if (!state.addedImports.has(importPath.node.source.value) && state.traversalState.treeshakeCompensation) {
            importPath.insertBefore(importDeclaration([], importPath.node.source));
    ...
    if (prop.isObjectProperty()) {
      ...
      if (valuePath.isObjectExpression()) {
        ...
        if (!result.confident) {
          ...
          const inlineStyleExpression = unit !== '' ? callExpression(arrowFunctionExpression([identifier('val')], conditionalExpression(binaryExpression('===', unaryExpression('typeof', identifier('val')), stringLiteral('number')), binaryExpression('+', identifier('val'), stringLiteral(unit)), conditionalExpression(binaryExpression('!=', identifier('val'), nullLiteral()), identifier('val'), identifier('undefined')))), [expression]) : conditionalExpression(binaryExpression('!=', expression, nullLiteral()), expression, identifier('undefined'));
...
function isSafeToSkipNullCheck(expr) {
  if (isTemplateLiteral(expr)) return true;
  if (isStringLiteral(expr) || isNumericLiteral(expr) || isBooleanLiteral(expr)) return true;
          ...
          if (key != null && Object.keys(fns).includes(key)) {
            ...
            if (isObjectExpression(prop.value)) {
              ...
              value.properties.forEach(prop => {
                if (!isObjectProperty(prop) || isPrivateName(prop.key)) {
                  return;
                }
                const objProp = prop;
                const propKey = isIdentifier(objProp.key) && !objProp.computed ? objProp.key.name : isStringLiteral(objProp.key) ? objProp.key.value : null;
                if (propKey == null) {
                ...
                const classList = isStringLiteral(objProp.value) ? objProp.value.value.split(' ') : [];
                let isStatic = true;
                const exprList = [];
                classList.forEach((cls, index) => {
                  ...
                  if (expr && !isSafeToSkipNullCheck(expr)) {
                    isStatic = false;
                    exprList.push(conditionalExpression(binaryExpression('!=', expr, nullLiteral()), stringLiteral(clsWithSpace), expr));
                  } else {
                    exprList.push(stringLiteral(clsWithSpace));
                  }
                });
                const joined = exprList.length === 0 ? stringLiteral('') : exprList.reduce((acc, curr) => binaryExpression('+', acc, curr));
                if (isStatic) {
                  staticProps.push(objectProperty(objProp.key, joined));
                } else {
                  conditionalProps.push(objectProperty(objProp.key, joined));
                }
              });
              let staticObj = null;
              let conditionalObj = null;
              if (staticProps.length > 0) {
                staticProps.push(objectProperty(stringLiteral('$$css'), cssTagValue));
                staticObj = objectExpression(staticProps);
              }
              if (conditionalProps.length > 0) {
                conditionalProps.push(objectProperty(identifier('$$css'), cssTagValue));
                conditionalObj = objectExpression(conditionalProps);
              }
              let finalFnValue = objectExpression(Object.entries(inlineStyles).map(([key, val]) => objectProperty(stringLiteral(key), val.expression)));
              if (staticObj != null || conditionalObj != null) {
                finalFnValue = arrayExpression([staticObj && hoistExpression(path, staticObj), conditionalObj, finalFnValue].filter(Boolean));
              }
              prop.value = arrowFunctionExpression(params, finalFnValue);
            }
          }
        }
        return prop;
...
function validateStyleXCreate(path) {
  ...
  const hasSpread = arg.properties.some(prop => isSpreadElement(prop));
  if (hasSpread) {
...
function transformStyleXCreateTheme(callExpressionPath, state) {
  ...
  if (callExpressionNode.callee.type === 'Identifier' && state.stylexCreateThemeImport.has(callExpressionNode.callee.name) || callExpressionNode.callee.type === 'MemberExpression' && callExpressionNode.callee.object.type === 'Identifier' && callExpressionNode.callee.property.type === 'Identifier' && callExpressionNode.callee.property.name === 'createTheme' && state.stylexImport.has(callExpressionNode.callee.object.name)) {
    ...
    const conditionalIdentity = value => value;
...
function transformStyleXDefineVars(callExpressionPath, state) {
  ...
  if (callExpressionNode.callee.type === 'Identifier' && state.stylexDefineVarsImport.has(callExpressionNode.callee.name) || callExpressionNode.callee.type === 'MemberExpression' && callExpressionNode.callee.property.name === 'defineVars' && callExpressionNode.callee.object.type === 'Identifier' && callExpressionNode.callee.property.type === 'Identifier' && state.stylexImport.has(callExpressionNode.callee.object.name)) {
    ...
    try {
      ...
      throw callExpressionPath.buildCodeFrameError(error instanceof Error ? error.message : messages.nonStaticValue('defineVars'), SyntaxError);
    }
    const [variablesObj, injectedStylesSansKeyframes] = defineVars(normalizedValue, {
      ...state.options,
...
function normalizeDefineVarsObject(variables, dependencyState) {
  const normalizedVariables = {};
  const dependencyMap = new Map();
  for (const [key, value] of Object.entries(variables)) {
    const deps = new Set();
    dependencyMap.set(key, deps);
    normalizedVariables[key] = normalizeDefineVarsValue(value, key, dependencyState, deps, true);
  }
  const keys = new Set(Object.keys(normalizedVariables));
  for (const [key, deps] of dependencyMap.entries()) {
    deps.delete('__varGroupHash__');
    for (const dependency of deps) {
      if (!keys.has(dependency)) {
        throw new DefineVarsValueError(messages.unknownDefineVarsReference(key, dependency));
      }
    }
  }
...
function normalizeDefineVarsValue(value, rootKey, dependencyState, dependencies, allowCSSType = false) {
  if (typeof value === 'function') {
    return evaluateDefineVarsFunction(value, rootKey, dependencyState, dependencies);
  }
  ...
  if (typeof value === 'object' && value != null) {
    if (value.default === undefined) {
      throw new DefineVarsValueError('Default value is not defined for ' + rootKey + ' variable.');
    }
    const normalizedValue = {};
    for (const [nestedKey, nestedValue] of Object.entries(value)) {
      normalizedValue[nestedKey] = normalizeDefineVarsValue(nestedValue, rootKey, dependencyState, dependencies);
    }
    return normalizedValue;
  }
  throw new DefineVarsValueError('Invalid value in defineVars');
}
function evaluateDefineVarsFunction(fn, rootKey, dependencyState, dependencies) {
  ...
  return normalizeDefineVarsValue(result, rootKey, dependencyState, dependencies);
}
function assertNoDefineVarsCycles(dependencyMap) {
...
function transformStyleXKeyframes(path, state) {
  ...
  if (nodeInit.callee.type === 'Identifier' && state.stylexKeyframesImport.has(nodeInit.callee.name) || nodeInit.callee.type === 'MemberExpression' && nodeInit.callee.object.type === 'Identifier' && nodeInit.callee.property.name === 'keyframes' && nodeInit.callee.property.type === 'Identifier' && state.stylexImport.has(nodeInit.callee.object.name)) {
    ...
    init.replaceWith(stringLiteral(animationName));
...
function transformStyleXPositionTry(path, state) {
  ...
  if (nodeInit.callee.type === 'Identifier' && state.stylexPositionTryImport.has(nodeInit.callee.name) || nodeInit.callee.type === 'MemberExpression' && nodeInit.callee.object.type === 'Identifier' && nodeInit.callee.property.name === 'positionTry' && nodeInit.callee.property.type === 'Identifier' && state.stylexImport.has(nodeInit.callee.object.name)) {
    ...
    init.replaceWith(stringLiteral(positionTryName));
...
function parseNullableStyle$1(node, state) {
  if (isNullLiteral(node) || isIdentifier(node) && node.name === 'undefined') {
    return null;
  }
...
function makeStringExpression$1(values) {
  ...
  return memberExpression(objExpressions, conditionsToKey, true);
}
...
function genBitwiseOrOfConditions$1(conditions) {
  const binaryExpressions = conditions.map((condition, i) => {
    ...
    return binaryExpression('<<', unaryExpression('!', unaryExpression('!', condition)), numericLiteral(shift));
  });
...
function transformStylexProps(path, state) {
  ...
  for (const argPath of argsPath) {
    currentIndex++;
    if (argPath.isObjectExpression() || argPath.isIdentifier() || argPath.isMemberExpression() || argPath.isCallExpression()) {
  ...
  if (bailOut) {
    ...
    for (const argPath of argumentPaths) {
      ...
      function ObjectExpression(objPath) {
        const hasCssMarker = objPath.node.properties.some(prop => isObjectProperty(prop) && (isIdentifier(prop.key) && prop.key.name === '$$css' || isStringLiteral(prop.key) && prop.key.value === '$$css') && isBooleanLiteral(prop.value, {
          value: true
        }));
        if (hasCssMarker) {
    ...
    if (path.parentPath.node.type === 'JSXSpreadAttribute') {
      if (isObjectExpression(stringExpression) && stringExpression.properties.length > 0 && stringExpression.properties.every(prop => isObjectProperty(prop) && (isIdentifier(prop.key) || isStringLiteral(prop.key)) && !prop.computed)) {
        const jsxAttributes = stringExpression.properties.filter(prop => isObjectProperty(prop)).map(prop => {
          ...
          let attributeValue;
          if (isStringLiteral(objectProp.value)) {
            attributeValue = objectProp.value;
          } else {
            attributeValue = stringLiteral(String(objectProp.value));
          }
          return jsxAttribute(jsxIdentifier(attrName), attributeValue);
        });
...
function transformStyleXViewTransitionClass(path, state) {
  ...
  if (nodeInit.callee.type === 'Identifier' && state.stylexViewTransitionClassImport.has(nodeInit.callee.name) || nodeInit.callee.type === 'MemberExpression' && nodeInit.callee.object.type === 'Identifier' && nodeInit.callee.property.name === 'viewTransitionClass' && nodeInit.callee.property.type === 'Identifier' && state.stylexImport.has(nodeInit.callee.object.name)) {
    ...
    init.replaceWith(stringLiteral(viewTransitionClassName));
    const injectedStyles = {
      ...otherInjectedCSSRules,
...
function isCallTo(node, importSet, memberName, stylexImport) {
  return node.callee.type === 'Identifier' && importSet.has(node.callee.name) || node.callee.type === 'MemberExpression' && node.callee.object.type === 'Identifier' && node.callee.property.type === 'Identifier' && node.callee.property.name === memberName && stylexImport.has(node.callee.object.name);
}
function validateDefineCall(callExpressionPath, apiName, argCount, requireExport = true) {
  const variableDeclaratorPath = callExpressionPath.parentPath;
  if (variableDeclaratorPath == null || variableDeclaratorPath.isExpressionStatement() || !variableDeclaratorPath.isVariableDeclarator() || variableDeclaratorPath.node.id.type !== 'Identifier') {
    throw callExpressionPath.buildCodeFrameError(messages.unboundCallValue(apiName), SyntaxError);
  }
  if (requireExport && !isVariableNamedExported(variableDeclaratorPath)) {
    throw callExpressionPath.buildCodeFrameError(messages.nonExportNamedDeclaration(apiName), SyntaxError);
  }
  if (callExpressionPath.node.arguments.length !== argCount) {
    throw callExpressionPath.buildCodeFrameError(messages.illegalArgumentLength(apiName, argCount), SyntaxError);
  }
}
function buildEvalConfig(state, otherInjectedCSSRules) {
  function keyframes$1(animation) {
    const [animationName, injectedStyle] = keyframes(animation, state.options);
    otherInjectedCSSRules[animationName] = injectedStyle;
    return animationName;
...
function transformStyleXDefineVarsNested(callExpressionPath, state) {
  const node = callExpressionPath.node;
  if (node.type !== 'CallExpression') return;
  if (!isCallTo(node, state.stylexDefineVarsNestedImport, 'unstable_defineVarsNested', state.stylexImport)) {
    return;
  }
  ...
  const {
    identifiers,
    memberExpressions
  } = buildEvalConfig(state, otherInjectedCSSRules);
  const {
    confident,
    value
  } = evaluate(firstArg, state, {
  ...
  if (!confident) {
    throw callExpressionPath.buildCodeFrameError(messages.nonStaticValue('unstable_defineVarsNested'), SyntaxError);
  }
  if (typeof value !== 'object' || value == null) {
    throw callExpressionPath.buildCodeFrameError(messages.nonStyleObject('unstable_defineVarsNested'), SyntaxError);
  }
  const fileName = state.fileNameForHashing;
  if (fileName == null) {
    throw new Error(messages.cannotGenerateHash('unstable_defineVarsNested'));
  }
  const [variablesObj, injectedStylesSansKeyframes] = defineVarsNested(value, {
    ...state.options,
    exportId: utils.genFileBasedIdentifier({
      fileName,
...
function transformStyleXDefineConstsNested(callExpressionPath, state) {
  const node = callExpressionPath.node;
  if (node.type !== 'CallExpression') return;
  if (!isCallTo(node, state.stylexDefineConstsNestedImport, 'unstable_defineConstsNested', state.stylexImport)) {
    return;
  }
  ...
  if (!confident) {
    throw callExpressionPath.buildCodeFrameError(messages.nonStaticValue('unstable_defineConstsNested'), SyntaxError);
  }
  if (typeof value !== 'object' || value == null) {
    throw callExpressionPath.buildCodeFrameError(messages.nonStyleObject('unstable_defineConstsNested'), SyntaxError);
  }
  const fileName = state.fileNameForHashing;
  if (fileName == null) {
  ...
  const [transformedJsOutput, jsOutput] = defineConstsNested(value, {
    ...state.options,
    exportId
  });
  callExpressionPath.replaceWith(convertObjectToAST(transformedJsOutput));
...
function transformStyleXCreateThemeNested(callExpressionPath, state) {
  const node = callExpressionPath.node;
  if (node.type !== 'CallExpression') return;
  if (!isCallTo(node, state.stylexCreateThemeNestedImport, 'unstable_createThemeNested', state.stylexImport)) {
    return;
  }
  validateDefineCall(callExpressionPath, 'unstable_createThemeNested', 2, false);
  const variableDeclaratorPath = callExpressionPath.parentPath;
  if (!variableDeclaratorPath.isVariableDeclarator()) return;
  const id = variableDeclaratorPath.get('id');
  ...
  if (!confident1) {
    throw callExpressionPath.buildCodeFrameError(messages.nonStaticValue('unstable_createThemeNested'), SyntaxError);
  }
  if (typeof variables.__varGroupHash__ !== 'string' || variables.__varGroupHash__ === '') {
    throw callExpressionPath.buildCodeFrameError('Can only override variables theme created with unstable_defineVarsNested().', SyntaxError);
  }
  const otherInjectedCSSRules = {};
  const {
    identifiers,
  ...
  if (typeof overrides !== 'object' || overrides == null) {
    throw callExpressionPath.buildCodeFrameError(messages.nonStyleObject('unstable_createThemeNested'), SyntaxError);
  }
  let [overridesObj, injectedStyles] = createThemeNested(variables, overrides, state.options);
  if (state.isTest) {
    const fileName = state.filename ?? 'UnknownFile';
    const basename = path.basename(fileName).split('.')[0];
    const devClassName = `${basename}__${variableName}`;
...
var babelTransform;
var hasRequiredBabelTransform;

function requireBabelTransform () {
	if (hasRequiredBabelTransform) return babelTransform;
	hasRequiredBabelTransform = 1;
...
var babelTransformExports = /*@__PURE__*/ requireBabelTransform();
    ...
    visitor: {
      Program: {
        ...
        exit: path => {
          ...
          const atomsVisitor = babelTransformExports.createUtilityStylesVisitor(state, {
            styleXCreateSet,
            convertObjectToAST,
            hoistExpression,
            injectDevClassNames
      ...
      JSXOpeningElement(path) {
        ...
        for (const attr of path.get('attributes')) {
          ...
          attr.replaceWith(jsxSpreadAttribute(callExpression(memberExpression(identifier(stylexLocalName), identifier('props')), [value.node])));
          break;
        }
      },
      CallExpression(path) {
...
function getStylexRuntimeBinding(path, state) {
  ...
  for (const localName of state.stylexImport) {
    ...
    if (programBinding != null && path.scope.getBinding(localName) === programBinding) {
      return localName;
    }
  }
  const existingImport = program.node.body.find(statement => statement.type === 'ImportDeclaration' && state.importSources.includes(statement.source.value));
  const existingImportSource = existingImport?.type === 'ImportDeclaration' ? existingImport.source.value : null;
  const importSource = existingImportSource ?? state.importSources[2] ?? state.importSources[0];
  const stylexLocalName = existingImportSource != null || path.scope.hasBinding('stylex') ? program.scope.generateUid('stylex') : 'stylex';
  const [importPath] = program.unshiftContainer('body', importDeclaration([undefined(identifier(stylexLocalName))], stringLiteral(importSource)));
