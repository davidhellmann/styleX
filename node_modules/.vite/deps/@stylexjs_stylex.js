...
function attrs(...styles) {
	const { className, style, "data-style-src": dataStyleSrc } = props.apply(this, styles);
	const result = {};
	if (className != null) result.class = className;
	if (style != null) result.style = Object.entries(style).map(([key, value]) => `${toKebabCase(key)}:${value}`).join(";");
	if (dataStyleSrc != null) result["data-style-src"] = dataStyleSrc;
	return result;
}
...
var legacyMerge = /*@__PURE__*/ function() {
